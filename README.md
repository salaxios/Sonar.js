...
What we can do instead: Write compatibility shims for individual common plugins to shift demanding JS logic to C. This will maximize compatibility and performance many in the general RPGMaker MZ community, however it will never guarantee universal performance for every individual case.

Thus, at core, Sonar.JS is a DIY native runtime framework core that you can use to compile your game for different platforms natively, think of it as a publically available barebones starting point for porting your games to different platforms. However it will require you to optimize your own custom plugins and write your own optimization code in C for the plugins that are not supported by the public version of the project. For most of you who just want to play shovelware off of itch.io, it should still work (barring the fact that effekseer animations will never show in battles).

If you just want to make a simple RPGMaker MZ game with minimal plugins and MV style battle animations instead of Effekseer, then this is pretty much drag and drop.
